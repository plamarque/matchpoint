...
- Le logo apparaît sur l’**affichage principal** dans chaque **panneau équipe**, **centré sous le nom** de la troupe ; une zone dédiée permet de le choisir (fichier image), sans libellé dans la zone (contrôle du même ordre que les autres boutons fantômes) ; les deux côtés utilisent la **même** présentation (dimensions de zone équivalentes).
